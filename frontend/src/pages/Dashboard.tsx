  ...
  if (user?.role === 'gate') {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl shadow-lg p-8 mb-8 flex flex-col md:flex-row items-center justify-between animate-fade-in">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Gate Security Dashboard</h1>
            <p className="text-lg text-blue-100">Welcome, {user.fullName || user.username}! Manage visitor check-ins and check-outs.</p>
          </div>
        ...
        <div className="mb-6 flex items-center gap-4 bg-white p-4 rounded-lg shadow">
          ...
          <label className="text-sm font-medium text-gray-700">Filter by Gate:</label>
          <select
            value={gateFilter}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-blue-100 rounded-lg shadow-md p-6 flex flex-col items-center animate-pop-in">
            <FaUserCheck className="text-4xl text-blue-600 mb-2" />
            <span className="text-3xl font-bold text-blue-600">{approvedVisitors.length}</span>
            <span className="text-sm font-medium text-gray-700 mt-2">Approved for Check-in</span>
          </div>
          <div className="bg-green-100 rounded-lg shadow-md p-6 flex flex-col items-center animate-pop-in">
            <FaUsers className="text-4xl text-green-600 mb-2" />
            <span className="text-3xl font-bold text-green-600">{checkedInVisitors.length}</span>
            <span className="text-sm font-medium text-gray-700 mt-2">Currently Checked In</span>
          </div>
          <div className="bg-purple-100 rounded-lg shadow-md p-6 flex flex-col items-center animate-pop-in">
            <FaUser className="text-4xl text-purple-600 mb-2" />
            <span className="text-3xl font-bold text-purple-600">{todayVisitors.length}</span>
            <span className="text-sm font-medium text-gray-700 mt-2">Today's Visitors</span>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Visitor</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Gate & Search</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Department</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Scheduled</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {[...approvedVisitors, ...checkedInVisitors].map(request => (
                  <tr key={request._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{request.visitorName}</div>
                        ...
                        {request.approvalCode && (
                          <div className="text-xs text-blue-600">Code: {request.approvalCode}</div>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="font-medium text-blue-600">{request.gateAssignment}</div>
                        <div className={`text-xs px-2 py-1 rounded-full inline-block ${
                          request.searchRequired === 'required' 
                            ? 'bg-red-100 text-red-800' 
                            : 'bg-green-100 text-green-800'
                        }`}>
                          {request.searchRequired === 'required' ? 'Search Required' : 'No Search'}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{request.department}</div>
                      <div className="text-xs text-gray-500">{request.requestedBy?.fullName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{new Date(request.scheduledDate).toLocaleDateString()}</div>
                      <div className="text-sm text-gray-500">{request.scheduledTime}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {request.status === "approved" && (
                        <button
                          onClick={() => handleGateCheckIn(request._id)}
                          disabled={actionLoading === request._id + "-checkin"}
                          className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 disabled:opacity-50 mr-2"
                        >
                          {actionLoading === request._id + "-checkin" ? "Checking In..." : "Check In"}
                        </button>
                      )}
                      {request.status === "checked_in" && (
                        <button
                          onClick={() => handleGateCheckOut(request._id)}
                          disabled={actionLoading === request._id + "-checkout"}
                          className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 disabled:opacity-50"
                        >
                          {actionLoading === request._id + "-checkout" ? "Checking Out..." : "Check Out"}
                        </button>
                      )}
                    </td>
                  </tr>
                ))}
                {[...approvedVisitors, ...checkedInVisitors].length === 0 && (
                  <tr><td colSpan={6} className="text-center text-gray-400 py-6">No visitors to manage at this time.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl shadow-lg p-8 mb-8 flex flex-col md:flex-row items-center justify-between animate-fade-in">
          <div>
            ...
            <p className="text-lg text-blue-100">Welcome, {user.fullName || user.username}! Monitor all visitor activities.</p>
          </div>
          <div className="mt-4 md:mt-0 text-white">
            <div className="text-2xl font-bold">{new Date().toLocaleDateString()}</div>
            <div className="text-sm">{new Date().toLocaleTimeString()}</div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden animate-fade-in">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-xl font-semibold">All Visitor Requests (Read-Only)</h2>
            <div className="text-sm text-gray-500">Total: {visitors.length} requests</div>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {visitors.slice(0, 20).map(request => (
                  <tr key={request._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{request.visitorName}</div>
                ...
                {visitors.length === 0 && (
                  <tr><td colSpan={5} className="text-center text-gray-400 py-6">No visitor requests found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
